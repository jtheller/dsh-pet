...
0.1.4将本地验证过的工作气泡余量实现同步进来；额度通过独立元数据传递，桌面/Web共用文本合成，通知占位和动画state key保持原规则。2026-09-24检查上游0.2.12：从0.2.11起9个提交、17个文件变化，包括素材fetch/blob与缓存、工作状态生命周期、helper宿主存活/管道及环境修复。本版暂不升级固定原包，避免未经适配就套用旧锚点；未来迁移需单独回归这些变化。
