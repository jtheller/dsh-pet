...
工作余量随补丁生效，无需重配动画或重新授权。可选肥鱼人设沿用0.1.3：在重启前运行 `npm run configure:persona` 启用，已有自定义人设可以跳过；`npm run restore:persona` 恢复此前人设。

上游已有0.2.12，本版仍要求0.2.11。不要先升级原版再套用本补丁；安装器会拒绝不匹配版本。
