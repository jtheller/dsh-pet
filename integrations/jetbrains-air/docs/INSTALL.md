...
退出 DSH，将新版 ZIP 中的文件覆盖到原扩展目录，**保留原目录的 `.local` 安装清单和恢复原件**。然后运行 `npm run prepare:upstream`、`npm run install:touch`、`npm run status`，重新启动 DSH 并刷新 Web。此版本仅改变菜单显示，不需要重新配置动画或账号。不要删除旧目录后再安装到新路径。
